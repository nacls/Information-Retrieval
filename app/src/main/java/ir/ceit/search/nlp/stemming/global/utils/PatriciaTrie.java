...
public class PatriciaTrie<K, V> extends AbstractMap<K, V> implements Trie<K, V>, Serializable {
    ...
    public String toString() {
        ...
        for(Iterator<Entry<K, V>> i = newEntryIterator(); i.hasNext(); ) {
    ...
    public Entry<K, V> traverse(Cursor<? super K, ? super V> cursor) {
        ...
        while(entry != null) {
            ...
            case REMOVE_AND_EXIT:
                Entry<K, V> value = new TrieEntry<K, V>(current.getKey(), current.getValue(), -1);
                removeEntry(current);
                return value;
    ...
    private static class TrieEntry<K,V> implements Entry<K,V>, Serializable {
    ...
    private class SingletonIterator implements Iterator<Entry<K, V>> {
    ...
    private class EntryIterator extends NodeIterator<Entry<K,V>> {
        @Override
        public Entry<K,V> next() {
            return nextEntry();
        }
    ...
    private class PrefixEntryIterator extends NodeIterator<Entry<K, V>> {
    ...
    private class SubMapEntryIterator extends NodeIterator<Entry<K,V>> {
    ...
    private class EntrySet extends AbstractSet<Entry<K,V>> {
        ...
        public boolean contains(Object o) {
            ...
            TrieEntry<K,V> candidate = getEntry(((Entry)o).getKey());
    ...
    private class PrefixSubMap extends SubMap {
        ...
        @Override
        protected Set<Entry<K, V>> newSubMapEntrySet() {
            return new PrefixEntrySetView();
        }

        private class PrefixEntrySetView extends EntrySetView {
